...
class HkubeAlgoMetrics extends StorageBase {
    ...
    async getMetricsPath({ pipelineName, nodeName, runName }) {
        let relativePath;
        if (runName) {
            relativePath = path.join(this.clusterName + '-' + STORAGE_PREFIX.HKUBE_ALGO_METRICS, pipelineName, nodeName, runName);
        }
        else {
            relativePath = path.join(this.clusterName + '-' + STORAGE_PREFIX.HKUBE_ALGO_METRICS, pipelineName, nodeName);
        }
