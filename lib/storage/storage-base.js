...
class StorageBase {
    ...
    async put(options, tracer, decodeOptions) {
        let span = null;
        try {
            const start = now();
            if (tracer) {
            ...
            const data = this._encoding.encode(options.data, decodeOptions);
    ...
    async get(options, tracer, decodeOptions) {
        ...
        try {
            ...
            const result = this._encoding.decode(data, decodeOptions);
            if (span) span.finish();
            const end = now();
            const diff = (end - start).toFixed(3);
            if (this._log) {
    ...
    async seek(options, tracer) {
        ...
        try {
            ...
            const diff = (end - start).toFixed(3);
            if (this._log) {
                this._log.debug(`seek - Execution time ${diff} ms`, { component, operation: 'seek', time: diff });
            }
            return result;
        }
        catch (error) {
