...
class StorageBase {
    ...
    constructor(adapter, log, encoding) {
        ...
        this.getCustomData = this._wrapper(this.getCustomData.bind(this));
    ...
    async getCustomData(options, trace) {
        const encodeOptions = options.encodeOptions || {};
        const { data, headerInData, metadata } = await this._adapter.getCustomData(options, trace);
        let payload = data;
        if (!headerInData && metadata.header) {
            payload = this._encoding.decodeHeaderPayload(metadata.header, data);
        }
        else {
            payload = this._encoding.decode(data, { ...encodeOptions, customEncode: true });
        }
    ...
    /** @returns {Promise<EntryWithMetaData[]>} */
     ...
     * @returns {Promise<number>} amount of files deleted
     * */
    async deleteFiles(files) {
        /** @type {{status: 'fulfilled' | 'rejected', value?:any, reason?: string}[]} */
        const result = await Promise.allSettled(files.map(file => this._adapter.delete(file)));
        return result.filter(item => item.status === 'fulfilled').length;
    }
