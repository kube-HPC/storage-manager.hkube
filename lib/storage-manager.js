...
class StorageManager {
    ...
    async getCustomFormat(options) {
        ...
        if (totalLength <= footerLength) {
            ...
            // read last bytes will get the footer
            const footer = await this.seek({ path, end: -footerLength });
            const magicNum = footer.slice(footer.length - magicNumber.length, footer.length).toString();
            isCustomFormat = magicNum === magicNumber;
            ...
            if (isCustomFormat) {
                ...
                if (dataType === 2) {
                    ...
                    stream = await this.getStream({ path, start: 0, end: totalLength - footerLength - 1 });
                }
            }
            else {
                stream = await this.getStream({ path });
        ...
        return { data, stream, size: totalLength, isCustomFormat };
    }
