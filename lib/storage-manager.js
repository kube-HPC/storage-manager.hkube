...
const HkubePersistency = require('./storage/hkube-persistency');
...
class StorageManager {
    ...
    async init(config, log, bootstrap = false) {
        if (!this._wasInit) {
            ...
            this.hkubePersistency = new HkubePersistency(this._adapter, log, config, this.encoding);
        }
    }
    ...
    async getCustomStream(options) {
        ...
        if (totalLength <= headerLength) {
            ...
            const { isCustomEncoding, isDataTypeEncoded } = await this._readFormat({ path, totalLength });
            if (isCustomEncoding) { // check if hkube encoding is here
                if (isDataTypeEncoded) { // check if this data is encoded
                    ...
                    if (result.error) {
                        ...
                        stream = await this.getStream({ path, start: headerLength, end: totalLength, totalLength });
                    }
                    else {
                    ...
                    stream = await this.getStream({ path, start: headerLength, end: totalLength, totalLength }); // this data is not encoded, we should stream it (without header)
                }
            }
            else {
                stream = await this.getStream({ path, totalLength });
    ...
    async _readFormat(options) {
        const { path, totalLength } = options;
        const { headerLength, magicNumber } = this.encoding;
        const header = await this.seek({ path, start: 0, end: headerLength, totalLength }); // read first bytes to get header
