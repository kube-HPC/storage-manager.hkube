...
class StorageManager {
    ...
    async init(config, log, bootstrap = false) {
        if (!this._wasInit) {
            ...
            await this._adapter.init(storage.connection, Object.values(STORAGE_PREFIX).map(x => config.clusterName + '-' + x), bootstrap);
            this._wasInit = true;
            this.storage = new StorageBase(this._adapter, log);
