    ...
    Object.entries(config.storageAdapters).forEach(([k, v]) => {
        ...
        EncodingTypes.forEach((e) => {
            describe(`test start with ${name} ${e}`, () => {
                ...
                describe(name + ':getCustomFormat', () => {
                    if (['redis', 'etcd'].indexOf(name) > -1) {
                        return;
                    }
                    it.only('get and put string', async () => {
                        const jobId = uuid();
                        const taskId = uuid();
                        const storageInfo = await storageManager.hkube.put({ jobId, taskId, data: 'gal-gadot' });// eslint-disable-line
                        const res1 = await storageManager.getCustomFormat(storageInfo);
                        const res2 = await storageManager.hkubeMetadata.get({ jobId, taskId });
                        expect('gal-gadot').to.equal(res1);
                        expect('gal-gadot').to.equal(res2);
                    });
